...
const editUser = asyncHandler(async (req, res) => {
  ...
  if (notEmpty) {
    if (
      !upname.trim().match(regexName) ||
      !getEmail.trim().match(regexEmail) ||
      !uppassword.trim().match(regexPassword)
    ) {
      res.status(400).send({
        message:
          "Please make sure to enter valid Full Name, Email and Password",
      });
    }
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const regexName = /^(?=.{3,20}$)(?![_.])(?!.*[_.]{2})[a-zA-Z ]+(?<![_.])$/;
